...
const slides = highlight.highlightsItems
const index = props.index
const currentIndex = ref(index)
const videoId = computed(() => {
    const item = slides.find(h => h.index === currentIndex.value)
...
function nextSlide() {
    ...
    const next = slides.find(s => s.index === currentIndex.value + 1)
        ...
        <div class="flex gap-[10px] items-center">
            ...
            <div class="relative w-[380px] [aspect-ratio:9/16] bg-[#000] rounded-[20px]">
                <div class="relative w-full h-full">
                    <KinescopePlayer id="videoPlayer" :video-id="videoId" class="absolute top-0 left-0 w-full h-full"
                        auto-play muted ref="kinescope" @ready="handleReady" />
                </div>
